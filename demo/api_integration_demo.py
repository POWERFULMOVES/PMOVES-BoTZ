...
It will:
1. Connect to the running MCP services
2. Test API requests through the gateway
3. Verify response data matches the LMS schema
4. Report any issues
"""
...
# MCP Service endpoints (updated to match actual running services)
MCP_SERVICES = {
    "docling-mcp": "http://localhost:3020",  # No /health endpoint, just check base URL
    "e2b-runner": "http://localhost:7071",   # No /health endpoint, just check base URL
    "vl-sentinel": "http://localhost:7072",  # No /health endpoint, just check base URL
...
def log(message):
    ...
    message = message.replace("✅", "[PASS]").replace("❌", "[FAIL]").replace("⚠️", "[WARN]")
    print(f"[{timestamp}] {message}")

def test_connection():
    """Test if we can connect to the MCP services"""
    log("=== Testing MCP Service Connection ===")
    
    for service_name, health_url in MCP_SERVICES.items():
        try:
            response = requests.get(health_url, timeout=5)
            # For docling-mcp, any response (even 404) means the service is running
            if service_name == "docling-mcp":
                if response.status_code in [200, 404]:
                    log(f"[PASS] {service_name} service is accessible (status: {response.status_code})")
                else:
                    log(f"[FAIL] {service_name} service returned status {response.status_code}")
            else:
                if response.status_code == 200:
                    log(f"[PASS] {service_name} service is accessible")
                else:
                    log(f"[FAIL] {service_name} service returned status {response.status_code}")
        except requests.exceptions.RequestException as e:
            log(f"[FAIL] Could not connect to {service_name} service: {e}")
    
    # Test if cipher memory is accessible (if running)
    try:
