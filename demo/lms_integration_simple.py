#!/usr/bin/env python3
"""
Simple LMS API Integration Demo for PMOVES-Kilobots
...
def test_connection():
    ...
    # Test docling-mcp service
    try:
        response = requests.get("http://localhost:3020", timeout=5)
        if response.status_code in [200, 404]:
            log(f"[PASS] docling-mcp service is accessible (status: {response.status_code})")
        else:
            log(f"[FAIL] docling-mcp service returned status {response.status_code}")
    except requests.exceptions.RequestException as e:
        log(f"[FAIL] Could not connect to docling-mcp service: {e}")
    
    # Test e2b-runner service
    try:
        response = requests.get("http://localhost:7071", timeout=5)
        if response.status_code in [200, 404]:
            log(f"[PASS] e2b-runner service is accessible (status: {response.status_code})")
        else:
            log(f"[FAIL] e2b-runner service returned status {response.status_code}")
    except requests.exceptions.RequestException as e:
        log(f"[FAIL] Could not connect to e2b-runner service: {e}")
    
    # Test vl-sentinel service
    try:
        response = requests.get("http://localhost:7072", timeout=5)
        if response.status_code in [200, 404]:
            log(f"[PASS] vl-sentinel service is accessible (status: {response.status_code})")
        else:
            log(f"[FAIL] vl-sentinel service returned status {response.status_code}")
    except requests.exceptions.RequestException as e:
        log(f"[FAIL] Could not connect to vl-sentinel service: {e}")
