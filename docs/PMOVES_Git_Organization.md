...
1. **Create the runner and start the configuration experience**
   ```bash
   ./config.sh --url https://github.com/POWERFULMOVES/PMOVES.AI --token BB5OGYEXII5PQTZKUDRZ5J3JCI76E
   ```
...
### Mini-Agent ARC & Provider-Native CLIs

- Many of the repositories above are designed to act as **provider-native mini agents**:
  - Each repo owns its own CLI, SDK usage, and provider integration (e.g., Archon for cloud runners, PMOVES.YT for media, PMOVES-DoX for documentation).
  - When integrated into PMOVES-BoTZ, these CLIs become part of the "mini-agent city" layer and can bootstrap or manage the services they represent.
- Within the BoTZ stack:
  - Cipher exposes MCP and A2A metadata (such as `/.well-known/agent.json`), and other agents will follow similar patterns.
  - The initial mission of the CLI agents, once core services and smoke tests are green, is to **catalog themselves and their capabilities**:
    - Enumerate agents, tools, and health endpoints.
    - Publish this catalog into PMOVES-DoX and related documentation so the full ARC of agents, tools, and services can be discovered and composed.
