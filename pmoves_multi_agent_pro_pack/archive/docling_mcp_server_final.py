...
async def run_http_server(host: str = "0.0.0.0", port: int = 3020):
    ...
    try:
        ...
        # For SSE transport, we need to start the server differently
        # The SSE transport handles the HTTP server setup
        ...
        # Start the transport (this will start the HTTP server)
...
Examples:
  ...
  python docling_mcp_server_final.py --transport stdio
  
  # Run with HTTP transport
  python docling_mcp_server_final.py --transport streamable-http --host 0.0.0.0 --port 3020
  
  # Run with custom host and port
  python docling_mcp_server_final.py --transport streamable-http --host localhost --port 8080
        """
    )
