"""
Integration health checker for PMOVES-BoTZ services.
...
class IntegrationHealth:
    ...
    async def check_tensorzero(self, timeout: float = 2.0) -> bool:
        ...
        if not AIOHTTP_AVAILABLE:
            logger.warning("aiohttp not available, cannot check TensorZero")
            return False
    ...
    async def check_gpu_orchestrator(self, timeout: float = 2.0) -> bool:
        ...
        if not AIOHTTP_AVAILABLE:
            logger.warning("aiohttp not available, cannot check GPU Orchestrator")
            return False
