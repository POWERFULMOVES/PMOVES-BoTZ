# PMOVES-BoTZ Universal Credentials Bootstrap Script
...
$BOTZ_ROOT = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
$ENV_LOCAL = Join-Path $BOTZ_ROOT ".env.local"
$ROOT_ENV = Join-Path $BOTZ_ROOT ".env"
...
# All managed credentials (BoTZ-specific)
$CREDENTIAL_KEYS = @(
    # LLM API Keys
    "VENICE_API_KEY",
    "OPENAI_API_KEY",
...
# Required for minimal BoTZ operation (MCP + Cipher)
$REQUIRED_KEYS = @("VENICE_API_KEY", "TENSORZERO_URL")

# Parent PMOVES.AI repo paths (local dev fallback)
$PARENT_PATHS = @(
    (Join-Path $BOTZ_ROOT "..\..\..\PMOVES.AI\pmoves\.env"),
    (Join-Path $BOTZ_ROOT "..\..\..\PMOVES.AI\pmoves\env.shared"),
    (Join-Path $BOTZ_ROOT "..\..\pmoves\.env"),
    (Join-Path $BOTZ_ROOT "..\..\pmoves\env.shared"),
...
Write-Host "=== PMOVES-BoTZ Universal Credentials Bootstrap ===" -ForegroundColor Cyan
Write-Host ""
...
if ($missing -eq 0) {
    ...
    Write-Host "  2. Add to parent PMOVES.AI repo env.shared" -ForegroundColor White
