...
# Prefer external configuration package if available, but fall back to a
# minimal, self-contained configuration model so the server can run in
# containers without extra Python packages.
try:
    ...
    @dataclass
    class ServerConfig:
        name: str = os.environ.get("DOCLING_MCP_SERVER__NAME", "docling-mcp")
        host: str = os.environ.get("DOCLING_MCP_SERVER__HOST", "0.0.0.0")
        port: int = int(os.environ.get("DOCLING_MCP_SERVER__PORT", "3020"))
        transport: TransportType = TransportType(
            os.environ.get("DOCLING_MCP_SERVER__TRANSPORT", "http")
        )

    @dataclass
    class LoggingConfig:
        name: str = os.environ.get("DOCLING_MCP_LOGGING__NAME", "docling-mcp")
        level: LogLevel = LogLevel(
            os.environ.get("DOCLING_MCP_LOGGING__LEVEL", "INFO")
        )
        output: str = os.environ.get("DOCLING_MCP_LOGGING__OUTPUT", "stdout")
        format: str = os.environ.get(
            "DOCLING_MCP_LOGGING__FORMAT",
            "%(asctime)s - %(name)s - %(levelname)s - %(message)s",
        )

    @dataclass
    class SseConfig:
        endpoint: str = os.environ.get("DOCLING_MCP_SSE__ENDPOINT", "/mcp")
        ...
        keepalive_interval: int = int(
            os.environ.get("DOCLING_MCP_SSE__KEEPALIVE_INTERVAL", "30")
        )

    @dataclass
    class HealthCheckConfig:
        endpoint: str = os.environ.get("DOCLING_MCP_HEALTH__ENDPOINT", "/health")
    ...
    def load_config(environment: str = "default", config_file: Optional[str] = None) -> "Config":
        """Fallback load_config when external config package is unavailable."""
        # Environment and config_file are currently ignored; everything is driven
        # by environment variables for container friendliness.
        return Config()
...
async def run_http_server(server: DoclingMCPServer) -> None:
    ...
    try:
        ...
        # Expose Prometheus metrics endpoint if exporter is enabled
        if getattr(server, 'metrics_exporter', None):
            try:
                prom_path = server.config.metrics.prometheus_endpoint if hasattr(server.config, 'metrics') else '/metrics'

                async def handle_prometheus_metrics(request: Request) -> Response:
                    try:
                        text = server.metrics_exporter.get_prometheus_metrics()
                        return web.Response(text=text, content_type='text/plain; version=0.0.4')
                    except Exception as e:
                        logger.error(f"Error generating Prometheus metrics: {e}")
                        return web.Response(text="", content_type='text/plain', status=500)

                app.router.add_get(prom_path, handle_prometheus_metrics)
                logger.info(f"Prometheus metrics endpoint configured at {prom_path}")
            except Exception as e:
                logger.error(f"Failed to configure Prometheus metrics endpoint: {e}")
        else:
            # Fallback minimal Prometheus /metrics when metrics package is unavailable
            try:
                prom_path = '/metrics'

                async def handle_prom(request: Request) -> Response:
                    text = (
                        "# HELP docling_mcp_up Docling MCP up status\n"
                        "# TYPE docling_mcp_up gauge\n"
                        "docling_mcp_up 1\n"
                    )
                    return web.Response(text=text, content_type='text/plain; version=0.0.4')

                app.router.add_get(prom_path, handle_prom)
                logger.info("Minimal Prometheus /metrics endpoint enabled (fallback)")
            except Exception as e:
                logger.error(f"Failed to enable fallback /metrics: {e}")
...
def main() -> None:
    ...
    try:
        if CONFIG_AVAILABLE:
            ...
            # Fallback to in-file default configuration
            config = Config()
            logger.warning(
                "Using built-in default configuration (external config system not available)"
            )
        # Initialize server with resolved configuration
