...
4. Mini CLI Command Hierarchy (Kilo-Bots Edition)
In the PMOVES mini CLI story, the core agents form a small "Kilo-Bots" squad with a clear chain of command:

Agent Zero - The Prime Orchestrator: Think of Agent Zero as the Optimus-class commander of the system. It sees the whole battlefield, decides which squads to deploy, and keeps long-term memory of prior missions, fixes, and architectural decisions. All other agents ultimately report up to Agent Zero.

Archon - Chief Architect & Command Tower: Archon is the command tower and archivist. It designs agents, curates prompts, and maintains the canonical registry of mini CLIs and PMOVES-Botz. When the system needs a new bot, a new workflow, or a refactor plan, Archon drafts the blueprint and hands it down to the field units (Crush, Cipher, etc.).

PMOVES-Crush - Field Commander of CLIs: Crush is the tactical field commander for CLIs - the one standing on the front line with your terminal. It routes requests to local and remote models, shows you the exact commands it would run, and works as the preferred local path to Ollama and other backends. Other CLIs stay first-class; Crush just coordinates them like a squad leader.

PMOVES-Cipher - Commander of PMOVES-Botz: Cipher is the captain of the PMOVES-Botz team. It manages memory, agent definitions, and the UI/API that front-loads those bots. When a mini CLI flow needs persistent memories, named agents, or workspace state, Cipher is the one that provisions and maintains those resources according to Archon's designs.

Reporting chain:
- PMOVES-Crush (CLIs) and PMOVES-Cipher (PMOVES-Botz) both report to PMOVES-Archon for architecture, prompts, and workflows.
- PMOVES-Archon reports to PMOVES-Agent-Zero, who integrates everything into a single self-hosted PMOVES city of Kilo-Bots.

5. Core Components and Functionality
5.1. Primary Orchestration & Learning Brain
...
5.2. Specialized Agent Building & Knowledge/Task Management
...
5.3. Workflow Orchestration & Inter-Agent Communication
...
5.4. Interactive CLI Agent for Software Engineering
